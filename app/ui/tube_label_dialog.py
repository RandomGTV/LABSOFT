...
class TubeLabelDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        tube_strip.setStyleSheet(f"background: {self.tube_color}; border-radius: 4px;")
        tl = QHBoxLayout(tube_strip)
        tl.setContentsMargins(12, 0, 12, 0)
        tb_label = QLabel(self.tube_name)
        tb_label.setStyleSheet("color: #ffffff; font-weight: 800; font-size: 9.5pt;")
        ...
        preview_box.setStyleSheet(
            "background: #ffffff; border: 2px dashed #94a3b8; border-radius: 6px; padding: 14px;"
        )
        ...
        pl.addWidget(QLabel(f"<b style='font-size:12pt; color:#0A3668;'>LAB REPORT #{report_no}</b>"))
        pl.addWidget(QLabel(f"<b style='font-size:11pt; color:#000;'>{patient_name}</b> ({patient_meta})"))
        pl.addWidget(QLabel(f"<span style='font-size:8.5pt; color:#475569;'>Date: {date_str}</span>"))

        # Barcode visual mock
        barcode_mock = QLabel("|| | |||| | ||| ||||| || | |||| ||| || | |||")
        barcode_mock.setStyleSheet("font-family: monospace; font-size: 18pt; letter-spacing: 2px; color: #000;")
        barcode_mock.setAlignment(Qt.AlignmentFlag.AlignCenter)
        pl.addWidget(barcode_mock)
